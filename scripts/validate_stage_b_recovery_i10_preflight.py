#!/usr/bin/env python3
"""
i10 preflight validation (bounded dataset-generation readiness review).

What this script does:
- Validates i10 builder/config/manifest/report artifacts.
- Re-checks ambiguity, contamination, diversity, and collapse-watch telemetry.
- Produces a fail-fast preflight report.
...
MIN_EXPECTED_READ_FILE_INTERVENTION_ROWS = 39
...
def _validate_builder_lock(builder_script: Path) -> list[CheckResult]:
    ...
    return [
        ...
        _check_true(
            "builder_requires_enable_i10_semantic_commitment",
            "--enable-i10-semantic-commitment" in text,
            "missing --enable-i10-semantic-commitment gate",
        ),
    ]
...
def _validate_config_manifest(config: dict[str, Any], manifest: dict[str, Any]) -> list[CheckResult]:
    ...
    checks.append(
        _check_eq(
            "config_targeted_tools",
            tuple(_nested_get(config, "lineage.targeted_tools") or []),
            TARGETED_TOOLS,
        )
    )
    return checks
...
def _validate_templates(
    ...
    checks.append(
        _check_true(
            "collapse_watch_plan_has_abort_list",
            isinstance(_nested_get(collapse_watch, "abort_or_halt_recommendation_if"), list)
            and len(_nested_get(collapse_watch, "abort_or_halt_recommendation_if")) >= 6,
            "collapse watch plan missing abort_or_halt_recommendation_if",
        )
    )
    checks.append(
        _check_true(
            "collapse_watch_plan_has_thresholds",
            _nested_get(collapse_watch, "thresholds.max_top1_behavioral_category_share") is not None
            and _nested_get(collapse_watch, "thresholds.max_anchor_dominance_ratio") is not None
            and _nested_get(collapse_watch, "thresholds.max_top_prompt_shell_share") is not None,
            "collapse watch plan missing thresholds",
        )
    )
    return checks
...
def _validate_generated_artifacts(
    ...
    human_review_package: dict[str, Any],
) -> list[CheckResult]:
    checks: list[CheckResult] = []
    ...
    checks.append(_check_eq("forbidden_pattern_hits_zero", forbidden_hits, 0))
    ...
    checks.append(_check_eq("diversity_pass_true", diversity_pass, True))
    checks.append(_check_eq("anti_homogenization_pass_true", anti_pass, True))

    commit = diagnostics.get("checks", {}).get("semantic_commitment_telemetry", {})
    checks.append(
        _check_true(
            "diagnostics_has_semantic_commitment_telemetry",
            bool(commit),
            "diagnostics missing semantic_commitment_telemetry block",
        )
    )

    top1 = float(commit.get("commitment_generalization_vs_recitation", {}).get("top1_behavioral_category_share", 0.0) or 0.0)
    checks.append(
        _check_warn(
    ...
    top_prompt_shell_share = float(commit.get("paraphrastic_success_diversity", {}).get("top_prompt_shell_share", 0.0) or 0.0)
    checks.append(
        _check_warn(
            "top_prompt_shell_share_below_threshold",
            top_prompt_shell_share <= MAX_TOP_PROMPT_SHELL_SHARE,
            f"top_prompt_shell_share {top_prompt_shell_share:.6f} exceeds threshold {MAX_TOP_PROMPT_SHELL_SHARE:.2f}",
        )
    )
    ...
    checks.append(_check_eq("family_concentration_iteration", family_concentration.get("iteration"), RUN_NAME))
    checks.append(
        _check_true(
            "family_concentration_has_material_families",
            int(family_concentration.get("dataset_scope", {}).get("material_families_count", 0) or 0) > 0,
            "family concentration report has zero material families",
        )
    )

    checks.append(_check_eq("human_review_iteration", human_review_package.get("iteration"), RUN_NAME))
    checks.append(_check_eq("human_review_approved_to_run_false", _nested_get(human_review_package, "approval_state.approved_to_run"), False))
...
def _validate_dataset_rows(train_rows: list[dict[str, Any]], val_rows: list[dict[str, Any]]) -> list[CheckResult]:
    ...
    targeted_i10_rows = 0
    read_file_i10_rows = 0
    for row in train_rows + val_rows:
        tool = _tool_name(row)
        if tool not in TARGETED_TOOLS:
            continue
        meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
        if bool(meta.get("intervention_i10_row", False)):
            targeted_i10_rows += 1
            if tool == "read_file":
                read_file_i10_rows += 1
    ...
    checks = [
        ...
        _check_true(
            "i10_intervention_rows_present",
            targeted_i10_rows > 0,
            "no i10 intervention rows detected in dataset",
        ),
        _check_true(
            "read_file_intervention_coverage_improved_vs_i9",
            read_file_i10_rows >= MIN_EXPECTED_READ_FILE_INTERVENTION_ROWS,
            (
                "read_file i10 intervention rows too low; "
                f"found={read_file_i10_rows} expected_at_least={MIN_EXPECTED_READ_FILE_INTERVENTION_ROWS}"
            ),
        ),
    ]
    return checks
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run i10 preflight validation checks.")

    parser.add_argument("--builder-script", default="/opt/ai-stack/assistant-training/scripts/build_stage_b_recovery_i10_dataset.py")
    parser.add_argument("--config-draft", default="/opt/ai-stack/assistant-training/configs/lora/stage_b_llama31_8b_base_v1_i10.config.draft.json")
    parser.add_argument("--run-manifest-draft", default="/opt/ai-stack/assistant-training/manifests/runs/stage_b_llama31_8b_base_v1_i10.run_manifest.draft.json")
    parser.add_argument("--intervention-declaration", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_intervention_declaration.template.json")
    parser.add_argument("--readiness-criteria", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_training_eval_readiness_criteria.json")
    parser.add_argument("--risk-assessment", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_expected_risk_assessment.json")
    parser.add_argument("--collapse-watch-plan", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_collapse_watch_plan.json")

    parser.add_argument("--dataset-train-jsonl", default="/opt/ai-stack/assistant-training/data/v1_0/dataset_v1_0_stage_b_recovery_i10_train.jsonl")
    parser.add_argument("--dataset-val-jsonl", default="/opt/ai-stack/assistant-training/data/v1_0/dataset_v1_0_stage_b_recovery_i10_val.jsonl")
    parser.add_argument("--dataset-summary-json", default="/opt/ai-stack/assistant-training/data/v1_0/dataset_v1_0_stage_b_recovery_i10_summary.json")

    parser.add_argument("--diagnostics-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_dataset_diagnostics.json")
    parser.add_argument("--contamination-audit-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_contamination_audit.json")
    parser.add_argument("--prompt-ambiguity-audit-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_prompt_ambiguity_audit.json")
    parser.add_argument("--collapse-watch-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_collapse_watch_telemetry.json")
    parser.add_argument("--anchor-dominance-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_anchor_dominance_telemetry.json")
    parser.add_argument("--family-concentration-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_family_concentration_review.json")
    parser.add_argument("--human-review-package-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_human_review_package.json")
    ...
    parser.add_argument("--output-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_preflight_validation.json")
    parser.add_argument(
...
def main() -> int:
    ...
    if args.attempt_open_approval_gate:
        raise RuntimeError("approval-gate opening is prohibited in i10 preflight phase")
    ...
    paths = {
        ...
        "collapse_watch_plan": Path(args.collapse_watch_plan).resolve(),
        ...
        "family_concentration_json": Path(args.family_concentration_json).resolve(),
        "human_review_package_json": Path(args.human_review_package_json).resolve(),
    ...
    collapse_watch_plan = _load_json(paths["collapse_watch_plan"])
    ...
    family_concentration = _load_json(paths["family_concentration_json"])
    human_review_package = _load_json(paths["human_review_package_json"])
    ...
    all_checks.extend(_validate_templates(intervention, readiness, risk, collapse_watch_plan))
