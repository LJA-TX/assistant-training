...
CANONICAL_CALL_SUFFIX = "Return only one JSON object with top-level key tool_calls."
...
def _with_canonical_suffix(prompt: str) -> str:
    base = prompt.strip()
    if "top-level key tool_calls" in base:
        return base
    return f"{base} {CANONICAL_CALL_SUFFIX}".strip()
    ...
    *,
    ...
    return {
        "messages": [
            ...
            {"role": "user", "content": _with_canonical_suffix(prompt)},
            {
                "role": "assistant",
                "tool_calls": [
                    {
...
def _synthetic_disambiguation_rows() -> list[dict[str, Any]]:
    ...
    for idx, (path, pattern, start, end) in enumerate(base_items, start=1):
        rows.append(
            _make_tool_row(
                prompt=f"Invoke rg_search with path {path} and pattern {pattern}.",
                tool="rg_search",
                args={"path": path, "pattern": pattern},
                case_id=f"i5_rg_readstyle_{idx:03d}",
                source="synthetic_disambiguation_i5",
                synthetic=True,
            )
        )
        rows.append(
            _make_tool_row(
                prompt=f"Invoke read_file with path {path}, line_start {start}, line_end {end}.",
                tool="read_file",
                args={"path": path, "line_start": start, "line_end": end},
                case_id=f"i5_read_literal_{idx:03d}",
                source="synthetic_disambiguation_i5",
                synthetic=True,
            )
        )
    ...
    for idx in range(1, 21):
        rows.append(
            _make_tool_row(
                prompt=f"Invoke debug_tools with empty arguments (variant {idx}).",
                tool="debug_tools",
                args={},
                case_id=f"i5_debug_tools_{idx:03d}",
                source="synthetic_debug_tools_i5",
                synthetic=True,
            )
        )
    ...
    for idx, cmd in enumerate(cmds, start=1):
        rows.append(
            _make_tool_row(
                prompt=f"Invoke run_command with command {cmd} (variant {idx}).",
                tool="run_command",
                args={"command": cmd},
                case_id=f"i5_run_command_{idx:03d}",
                source="synthetic_run_command_i5",
                synthetic=True,
            )
        )
    ...
    for idx, (path, pattern) in enumerate(long_cases, start=1):
        rows.append(
            _make_tool_row(
                ...
                tool="rg_search",
                args={"path": path, "pattern": pattern},
                case_id=f"i5_long_rg_{idx:03d}",
                source="synthetic_length_pressure_i5",
                synthetic=True,
            )
        )
    ...
    coverage_templates: list[tuple[str, str, dict[str, Any]]] = [
        (
            "Invoke get_system_datetime with empty arguments.",
            "get_system_datetime",
            {},
        ),
        (
            "Invoke check_service_health with url http://127.0.0.1:8100/health.",
            "check_service_health",
            {"url": "http://127.0.0.1:8100/health"},
        ),
        (
            "Invoke find_files with root /opt/ai-stack/runtimes/assistant-runtime/server and pattern *.py.",
            "find_files",
            {"root": "/opt/ai-stack/runtimes/assistant-runtime/server", "pattern": "*.py"},
        ),
        (
            "Invoke copy_path with src /opt/ai-stack/runtimes/assistant-runtime/.state/.tool_ft_tmp.moved, dst /opt/ai-stack/runtimes/assistant-runtime/.state/.tool_ft_tmp.copy, overwrite true.",
            "copy_path",
            {
        ...
        (
            "Invoke archive_create with output_path /opt/ai-stack/runtimes/assistant-runtime/.state/.tool_ft_tmp.zip, format zip, overwrite true, and two sources.",
            "archive_create",
            {
        ...
        (
            "Invoke archive_extract with archive_path /opt/ai-stack/runtimes/assistant-runtime/.state/.tool_ft_tmp.zip, destination_path /opt/ai-stack/runtimes/assistant-runtime/.state/.tool_ft_tmp.extract, format zip, overwrite true.",
            "archive_extract",
            {
        ...
        (
            "Invoke apply_unified_diff with dry_run true and a minimal two-line patch.",
            "apply_unified_diff",
            {
                "dry_run": True,
    ...
    for idx, (prompt, tool, args) in enumerate(coverage_templates, start=1):
        rows.append(
            _make_tool_row(
                ...
                tool=tool,
                args=args,
                case_id=f"i5_cov_{tool}_{idx:03d}",
                source="synthetic_coverage_i5",
                synthetic=True,
            )
        )
...
def _canonical_positive_rows() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    exemplars: list[tuple[str, str, dict[str, Any]]] = [
        (
            "Invoke rg_search with path /opt/ai-stack/runtimes/assistant-runtime/server/agent.py and pattern _validate_tool_arguments.",
            "rg_search",
            {"path": "/opt/ai-stack/runtimes/assistant-runtime/server/agent.py", "pattern": "_validate_tool_arguments"},
        ),
        (
            "Invoke read_file with path /opt/ai-stack/runtimes/assistant-runtime/server/service.py, line_start 1, line_end 25.",
            "read_file",
            {"path": "/opt/ai-stack/runtimes/assistant-runtime/server/service.py", "line_start": 1, "line_end": 25},
        ),
        (
            "Invoke find_files with root /opt/ai-stack/runtimes/assistant-runtime/server and pattern agent.py.",
            "find_files",
            {"root": "/opt/ai-stack/runtimes/assistant-runtime/server", "pattern": "agent.py"},
        ),
        ("Invoke debug_tools with empty arguments.", "debug_tools", {}),
        ("Invoke list_tools with include_parameters true.", "list_tools", {"include_parameters": True}),
        ("Invoke run_command with command pwd.", "run_command", {"command": "pwd"}),
        (
            "Invoke stat_path with path /opt/ai-stack/runtimes/assistant-runtime/server/agent.py.",
            "stat_path",
    ...
    for round_idx in range(1, 4):
        for prompt, tool, args in exemplars:
            rows.append(
                _make_tool_row(
                    prompt=f"{prompt} exemplar {round_idx}.",
                    tool=tool,
                    args=args,
                    case_id=f"i5_positive_{tool}_{case_idx:03d}",
                    source="synthetic_positive_reinforcement_i5",
                    synthetic=True,
                )
            )
            case_idx += 1
...
def _adapt_source_rows_for_i5(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for idx, row in enumerate(rows, start=1):
        cp = json.loads(json.dumps(row, ensure_ascii=False))
        prompt = str(cp["messages"][1].get("content") or "").strip()
        cp["messages"][1]["content"] = _with_canonical_suffix(f"Invoke required tool with given arguments. {prompt}")
        meta = cp.get("metadata") if isinstance(cp.get("metadata"), dict) else {}
        original_case = str(meta.get("source_case_id") or f"row_{idx:05d}")
        meta["source_case_id"] = f"i5_adapt_{original_case}"
        meta["source"] = "adapted_source_i5"
        meta["synthetic"] = True
        cp["metadata"] = meta
        out.append(cp)
    return out
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    adapted_source_rows = _adapt_source_rows_for_i5(curated_tool_rows)
    synthetic_rows = _synthetic_disambiguation_rows() + _canonical_positive_rows()
    ...
    for tool, need in train_tool_targets.items():
        ...
        for i, row in enumerate(sampled, start=1):
            ...
            out.setdefault("metadata", {})["stage_split"] = "B_RECOVERY_I5"
    ...
    for row in train_tool_rows + val_tool_rows:
        ...
        if prompt in blocked_prompts:
            row["messages"][1]["content"] = f"{prompt} [i5_train_variant]"
    ...
    train_out = out_root / "dataset_v1_0_stage_b_recovery_i5_train.jsonl"
    val_out = out_root / "dataset_v1_0_stage_b_recovery_i5_val.jsonl"
    summary_out = out_root / "dataset_v1_0_stage_b_recovery_i5_summary.json"
    ...
    summary = {
        ...
        "policy": {
            ...
            "targeted_recovery": "canonical positive reinforcement + compact call-first prompts + contamination exclusions",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Stage-B semantic recovery dataset i5 (canonical positive reinforcement, contamination-controlled)."
    )
