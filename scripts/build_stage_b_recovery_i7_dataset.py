...
ARGUMENTS_OBJECT_ANCHOR = "Return one JSON object with tool_calls; function.arguments must be a JSON object."
...
def _with_arguments_object_anchor(prompt: str) -> str:
    base = prompt.strip()
    if "function.arguments must be a JSON object" in base:
        return base
    return f"{base} {ARGUMENTS_OBJECT_ANCHOR}".strip()
    ...
    *,
    ...
    return {
        "messages": [
            ...
            {"role": "user", "content": _with_arguments_object_anchor(prompt)},
            {
                "role": "assistant",
                "tool_calls": [
                    {
    ...
    for idx, (path, pattern, start, end) in enumerate(base_items, start=1):
        rows.append(
            _make_tool_row(
                ...
                case_id=f"i7_rg_readstyle_{idx:03d}",
                source="synthetic_disambiguation_i7",
                synthetic=True,
            )
        )
        rows.append(
            _make_tool_row(
                ...
                case_id=f"i7_read_literal_{idx:03d}",
                source="synthetic_disambiguation_i7",
                synthetic=True,
            )
        )
    ...
    for idx in range(1, 21):
        rows.append(
            _make_tool_row(
                ...
                tool="debug_tools",
                args={},
                case_id=f"i7_debug_tools_{idx:03d}",
                source="synthetic_debug_tools_i7",
                synthetic=True,
            )
        )
    ...
    for idx, cmd in enumerate(cmds, start=1):
        rows.append(
            _make_tool_row(
                ...
                tool="run_command",
                args={"command": cmd},
                case_id=f"i7_run_command_{idx:03d}",
                source="synthetic_run_command_i7",
                synthetic=True,
            )
        )
    ...
    for idx, (path, pattern) in enumerate(long_cases, start=1):
        rows.append(
            _make_tool_row(
                ...
                tool="rg_search",
                args={"path": path, "pattern": pattern},
                case_id=f"i7_long_rg_{idx:03d}",
                source="synthetic_length_pressure_i7",
                synthetic=True,
            )
        )
    ...
    for idx, (prompt, tool, args) in enumerate(coverage_templates, start=1):
        rows.append(
            _make_tool_row(
                ...
                tool=tool,
                args=args,
                case_id=f"i7_cov_{tool}_{idx:03d}",
                source="synthetic_coverage_i7",
                synthetic=True,
            )
        )
...
def _payload_object_shape_rows() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    templates: list[tuple[str, str, dict[str, Any]]] = [
        ("Invoke debug_tools with empty object arguments only.", "debug_tools", {}),
        ("Invoke get_system_datetime with empty object arguments only.", "get_system_datetime", {}),
        ("Invoke list_tools with include_parameters true in object arguments.", "list_tools", {"include_parameters": True}),
        ("Invoke run_command with object arguments command=pwd.", "run_command", {"command": "pwd"}),
        (
            "Invoke stat_path with object argument path /opt/ai-stack/runtimes/assistant-runtime/server/agent.py.",
            "stat_path",
            {"path": "/opt/ai-stack/runtimes/assistant-runtime/server/agent.py"},
        ),
        (
            "Invoke rg_search with object arguments path and pattern for _validate_tool_arguments.",
            "rg_search",
            {"path": "/opt/ai-stack/runtimes/assistant-runtime/server/agent.py", "pattern": "_validate_tool_arguments"},
        ),
        (
            "Invoke read_file with object arguments path line_start line_end.",
            "read_file",
    ...
    for rep in range(1, 4):
        for prompt, tool, args in templates:
            rows.append(
                _make_tool_row(
                    prompt=f"{prompt} variant {rep}.",
                    tool=tool,
                    args=args,
                    case_id=f"i7_payload_object_{tool}_{case_idx:03d}",
                    source="synthetic_payload_object_i7",
                    synthetic=True,
                )
            )
            case_idx += 1
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    adapted_source_rows = _adapt_source_rows_for_i7(curated_tool_rows)
    synthetic_rows = _synthetic_disambiguation_rows() + _payload_object_shape_rows()
    ...
    for tool, need in train_tool_targets.items():
        ...
        for i, row in enumerate(sampled, start=1):
            ...
            out.setdefault("metadata", {})["stage_split"] = "B_RECOVERY_I7"
    ...
    for row in train_tool_rows + val_tool_rows:
        ...
        if prompt in blocked_prompts:
            row["messages"][1]["content"] = f"{prompt} [i7_train_variant]"
        else:
            row["messages"][1]["content"] = _with_arguments_object_anchor(prompt)
    ...
    train_out = out_root / "dataset_v1_0_stage_b_recovery_i7_train.jsonl"
    val_out = out_root / "dataset_v1_0_stage_b_recovery_i7_val.jsonl"
    summary_out = out_root / "dataset_v1_0_stage_b_recovery_i7_summary.json"
    ...
    summary = {
        ...
        "policy": {
            ...
            "targeted_recovery": "payload_not_object isolation via arguments-object anchor; other dimensions held invariant",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Stage-B semantic recovery dataset i7 (payload_not_object isolation, contamination-controlled)."
    )
