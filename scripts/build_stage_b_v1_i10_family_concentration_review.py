#!/usr/bin/env python3
"""
Build family concentration telemetry artifacts for Stage B i10.
...
from i10_diagnostics_scaffold import build_intervention_annotations
...
def build_review(args: argparse.Namespace) -> dict[str, Any]:
    ...
    i10_train = _load_jsonl(Path(args.i10_train_jsonl).resolve())
    i10_val = _load_jsonl(Path(args.i10_val_jsonl).resolve())
    i3_train = _load_jsonl(Path(args.i3_train_jsonl).resolve())
    i3_val = _load_jsonl(Path(args.i3_val_jsonl).resolve())

    i10_rows = i10_train + i10_val
    i3_rows = i3_train + i3_val

    anns = build_intervention_annotations(i10_rows, targeted_tools=TARGETED_TOOLS)

    by_family_anns: dict[str, list[dict[str, Any]]] = defaultdict(list)
    targeted_rows: list[dict[str, Any]] = []
    for idx, row in enumerate(i10_rows, start=1):
        tool = _assistant_tool_name(row)
        if tool not in TARGETED_TOOLS:
            continue
...
def _write_markdown(report: dict[str, Any], out_path: Path) -> None:
    lines: list[str] = []
    lines.append("# Stage B i10 Family Concentration Review")
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Stage B i10 family concentration review artifacts.")
    parser.add_argument("--i10-train-jsonl", default="/opt/ai-stack/assistant-training/data/v1_0/dataset_v1_0_stage_b_recovery_i10_train.jsonl")
    parser.add_argument("--i10-val-jsonl", default="/opt/ai-stack/assistant-training/data/v1_0/dataset_v1_0_stage_b_recovery_i10_val.jsonl")
    ...
    parser.add_argument("--output-json", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_family_concentration_review.json")
    parser.add_argument("--output-md", default="/opt/ai-stack/assistant-training/manifests/reports/stage_b_v1_i10_family_concentration_review.md")
