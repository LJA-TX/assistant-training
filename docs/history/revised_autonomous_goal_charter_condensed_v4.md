# Codex `/goal` Charter — Condensed Ingestion Version (v4)
...
**Conclusion**:
Tool-call SFT alone is insufficient. Runtime-oriented instruct alignment is required in addition to tool specialization.
...
Default operational preference:
- begin with Llama-3.1-8B-Instruct unless evidence strongly favors base;
- runtime-oriented instruct alignment + tool specialization are both authorized training objectives.
...
Codex may:
- perform runtime-oriented instruct tuning;
- perform tool-call specialization;
- perform staged or mixed curriculum training;
- mix instruct-alignment and tool-call datasets when beneficial.

The objective is a disciplined runtime assistant, not merely a chatbot with tools.
...
Canonicalize external tool-call formats into a single assistant-runtime schema before training whenever practical.
...
Default strategic roles:
- xLAM/APIGen → primary tool-call correctness data
- When2Call → no-call / tool-decision behavior
- ToolACE → later-stage diversity/generalization
- Glaive-style mixed data → anti-tool-happy balancing
...
Do not train the model toward unconditional tool usage.
Preserve strong direct-answer and no-call behavior where appropriate.
