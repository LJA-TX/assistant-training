# Codex `/goal` Charter — Further Condensed (v4c)
...
The project intentionally favors a cleaner behavioral substrate over inherited chatbot priors, accepting additional curriculum complexity in exchange for stronger long-term runtime discipline.
...
**Operational Objective**: Construct disciplined runtime-assistant behavior through staged runtime instruct alignment and tool specialization.
...
Codex may perform runtime-oriented instruct tuning, tool-call specialization, staged or mixed curriculum training, and mix runtime-alignment + tool-call datasets when beneficial.
...
Codex may autonomously inspect repos, create scripts/tests/builders/transforms, review/filter/fetch/download datasets, canonicalize schemas, convert formats, generate synthetic/paraphrased runtime data, run validations/preflights/training/eval, iterate configs/datasets, discard weak adapters, and write reports/recommendations — within reasonable local compute/storage constraints.
...
Aggressively filter or down-weight datasets with excessive conversational padding, assistant socialization, or wrapper-heavy behavior. Prefer datasets demonstrating concise, execution-oriented assistant behavior.

### High-Priority Candidate Dataset Families
Codex is explicitly authorized to investigate and use filtered/canonicalized subsets of:
...
Dataset should include: runtime-oriented instruct examples, tool-call positives, tool/no-tool discrimination, safe failure behavior, policy/no-call examples, and JSON-format discipline.
