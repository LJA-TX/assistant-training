# Appendix A — Operational Execution Contract (v2)
...
Canonical evaluation assets must be frozen through a single versioned manifest artifact.
...
Preferred tolerance:
- ±5 percentage points per category
...
surround or contaminate the canonical tool-call output.
...
Because the canonical target is a base model, Stage A is expected to establish foundational runtime-assistant behavior rather than merely refine existing assistant priors.
