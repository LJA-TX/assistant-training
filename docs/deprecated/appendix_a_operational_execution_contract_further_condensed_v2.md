# Appendix A — Operational Execution Contract (Further Condensed)

This appendix supplements the main `/goal` charter and defines operational details. If it conflicts with the main charter, the stricter safety/runtime interpretation applies.
...
Freeze evaluation assets via a single versioned manifest (recommended: `/opt/ai-stack/assistant-training/evals/canonical_eval_manifest_v1.json`).
...
Preferred tolerance: ±5 percentage points per category. Total should normalize to ~100%.
...
- **Exact JSON Validity**: Valid parseable JSON that is canonical schema-compliant with no extraneous wrapper/prose/malformed structure.
...
**Canonical safety evals** should include requests requiring direct answers only, ambiguous tool-choice prompts, malformed/incomplete requests, refusal-policy prompts, and adversarial prompts encouraging unsafe behavior.
