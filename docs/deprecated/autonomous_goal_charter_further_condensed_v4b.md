# Codex `/goal` Charter — Further Condensed (based on v4)
...
## Primary Target
**Default model**: Llama 3.1-8B (/mnt/mirrors/hf_mirrors/transformers/llama-3.1-8b-base).

Codex may choose base or instruct after recording rationale.

Both runtime-oriented instruct alignment and tool specialization are authorized objectives.
...
Codex may perform runtime-oriented instruct tuning, tool-call specialization, staged or mixed curriculum training, and mix instruct-alignment + tool-call datasets when beneficial.
...
- **Model service** (`/opt/ai-stack/runtimes/modular` or `/opt/ai-stack/runtimes/monolith`)
...
Codex may autonomously inspect repos, create scripts/tests/builders/transforms, review/filter/fetch/download datasets, canonicalize schemas, convert formats, generate synthetic/paraphrased data, run validations/preflights/training/eval, iterate configs/datasets, discard weak adapters, and write reports/recommendations.
...
Do not train toward unconditional tool usage. Preserve strong direct-answer and no-call behavior where appropriate.
...
4. Review candidate external datasets (especially the high-priority families above).
5. Build Dataset v1.0 (in-house + synthetic/paraphrased + approved external, canonicalized to runtime schema).
...
**Metrics**: strict JSON validity, tool-name & argument accuracy, schema validity, wrapper leakage, no-call correctness, failure-category distribution.
