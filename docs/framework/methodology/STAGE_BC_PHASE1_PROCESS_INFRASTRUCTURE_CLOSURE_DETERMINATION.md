...
- the canonical location of the validated Phase 1 process assets is now `docs/framework/process_infrastructure/...`
- preserved references to `docs/process_infrastructure/...` in this record describe the closure-time layout that was validated during Stage BC Phase 1 and remain compatibility-preserved through exact-file aliases
...
| Route references path-normalized | pass | At closure time, route table primary assets were concrete repo-relative `docs/process_infrastructure/...` paths. |
| Route asset references resolve | pass | At closure time, backtick-extracted `.md` references from `AGENTS.md` resolved to existing files under the validated layout. |
| Checklist library complete for Phase 1 | pass | At closure time, `docs/process_infrastructure/checklists` contained 8 `_checklist.md` assets. |
| Template library complete for Phase 1 | pass | At closure time, `docs/process_infrastructure/templates` contained 9 `_template.md` assets. |
...
2. at closure time, all referenced process assets existed under `docs/process_infrastructure`, and the canonical copies now reside under `docs/framework/process_infrastructure`;
